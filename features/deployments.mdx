...
sequenceDiagram
    ...
    User->>Karada: Configure Spec / Project
    Karada->>Runtime: Build & Containerize
    Runtime-->>Karada: Deploy Container & Generate Endpoint
    Karada-->>User: Secure HTTPS URL Generated
    User->>AIClient: Provide URL to AI
    AIClient<-->>Runtime: LLM uses the deployed MCP server
```
